...
>(
  ...
  const effects: { [key: string]: AsyncThunk<any, any, any> } = {};
  const loadings: { [key: string]: string } = {};
  if (options.effects) {
    Object.keys(options.effects).forEach((prefix: any) => {
      const type = options.name + "/" + (prefix as string);
